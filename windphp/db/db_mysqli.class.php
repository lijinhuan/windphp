...
class db_mysqli implements db_interface  {
	 ...
	 * 获取一条数据返回数组
	 */
	public function fetch_one($table,$data=array()){
		$data['limit'] = 1;
		$sql = $this->_formatSql($table,$data);
		$re = $this->query($sql);
	 ...
	 * 获取所有结果集
	 */
	function fetch_all($table='',$data=array()){
		$sql = $this->_formatSql($table, $data);
		$res = $this->query($sql);
		if ($res !== false){
			$arr = array();
	...
	public function update($table='',$data=array()){
		...
		$sql = $this->_formatSql($table, $data,'UPDATE');
		$re =  $this->query($sql);
		if($re){
	...
	public function delete($table='',$data=array()){
		...
		$sql = $this->_formatSql($table, $data,'DELETE');
	...
	public function insert($table='',$data=array()){
		$sql = $this->_formatSql($table, $data,'INSERT');
		$re = $this->query($sql);
		if($re){
	...
	public function replace($table='',$data=array()){
		$sql = $this->_formatSql($table, $data,'REPLACE');
		return $this->query($sql);
	}
	...
	private function _formatSql($table,$data,$type='SELECT'){
		if(empty($table)){
			throw new Exception("sql table empty ！");
		}
		$where = (isset($data['where']) and !empty($data['where']))?' WHERE '.$this->_formatWhere($data['where']):'';
		$set = (isset($data['set']) and !empty($data['set']))?' '.$this->_formatSet($data['set']):'';
	...
	private function _formatSet($keys){
		$set = '';
		$c = ' , ';
		foreach ($keys as $k=>$v){
			if(is_numeric($v)){
	...
	private function _formatWhere($keys=array()){
