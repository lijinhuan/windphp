...
.bg_btn{background: url(<!--{$system_conf['image_url']}-->admin_img/icon2.jpg) no-repeat; width:32px; height:32px;}
</style>
...
<div class="header">
    ...
    <div class="rt-col" style="width:auto;">
    	<div class="tab_style white cut_line text-r">
   				<a href="<!--{funcecho \Windphp\Core\UrlRoute::getWebUrl('Index-Run')}-->" style="color:white;margin-right:30px;">后台首页</a>   您好！<!--{$user['realname']}-->  [<!--{php echo implode(',',$user['rolename'])}-->]<span>|</span><a href="?controller=Login&action=Out">[退出]</a>
    	</div>	
    </div>
    <div class="col-auto">
        ...
        <!--{loop $top_menu $tmenu}-->
		     <li id="_M<!--{$tmenu['id']}-->" class="top_menu"><a href="javascript:_M(<!--{$tmenu['id']}-->,'javascript://')"  hidefocus="true" style="outline:none;"><!--{$tmenu['name']}--></a></li>    	
        <!--{/loop}-->
        </ul>
    </div>
</div>
...
$(function(){
	//默认载入左侧菜单
	$("#leftMain").load("index.php?controller=Index&action=Left&menuid=<!--{$top_menu[0]['id']}-->");
})
