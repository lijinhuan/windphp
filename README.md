...
###一、说明
    ...
    （5）项目目录自动创建，减少手动创建目录文件烦恼，只需第一次访问加载框架即可自动创建
    （6）支持多个数据库分别操作，多个缓存实例分别操作
    （7）内置模板解析引擎，可以使用原生php语法也支持模板分离
    （8）支持swoole的http，tcp，webSocket服务创建，同时可以方便使用windphp提供的框架服务
    （9）debug，exception，日志，翻页，http请求等常用开发组件支持
    （10）简单方便的魔法操作，使用该框架，你会发现各种操作真的很简单
     （11）安全，对各种输入进行了处理，目录安全隔离等
